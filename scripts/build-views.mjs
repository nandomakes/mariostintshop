...
// View tokens are matched by prefix so spelling variants work
// (FRONT, FRONTAL, REAR, REARR, ...).
const viewOf = (t) => (/^front/.test(t) ? 'front' : /^rear/.test(t) ? 'rear' : null);
...
function parseName(file) {
  ...
  const view = tokens.map(viewOf).find(Boolean);
...
// Compact has no renders of its own — it reuses the SUV (Model X) views.
if (entries.suv && !entries.compact) {
  entries.compact = entries.suv;
  console.log('✓ compact → aliased to suv views');
}
